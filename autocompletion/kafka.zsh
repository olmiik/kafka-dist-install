...
declare -a _connect_standalone_args
_connect_standalone_args=()
compdef "_kafka-command connect-standalone" connect-standalone
declare -a _ksql_stop_args
_ksql_stop_args=()
compdef "_kafka-command ksql-stop" ksql-stop
...
declare -a _kafka_replica_verification_args
_kafka_replica_verification_args=()
_kafka_replica_verification_args+=('--broker-list: REQUIRED: The list of hostname and port of the server to connect to.')
_kafka_replica_verification_args+=('--fetch-size: The fetch size of each request. (default: 1048576)')
_kafka_replica_verification_args+=('--help: Print usage information.')
_kafka_replica_verification_args+=('--max-wait-ms: The max amount of time each fetch request waits. (default: 1000)')
_kafka_replica_verification_args+=('--report-interval-ms: The reporting interval. (default: 30000)')
_kafka_replica_verification_args+=('--time: Timestamp for getting the initial offsets. (default: -1)')
_kafka_replica_verification_args+=('--topic-white-list: White list of topics to verify replica consistency. Defaults to all topics. (default: .*)')
_kafka_replica_verification_args+=('--version: Print version information and exit.')
compdef "_kafka-command kafka-replica-verification" kafka-replica-verification
...
declare -a _trogdor_args
_trogdor_args=()
compdef "_kafka-command trogdor" trogdor
declare -a _connect_mirror_maker_args
_connect_mirror_maker_args=()
_connect_mirror_maker_args+=('--clusters: --clusters CLUSTER [CLUSTER ...] Target cluster to use for this node.')
compdef "_kafka-command connect-mirror-maker" connect-mirror-maker
declare -a _kafka_rest_stop_args
_kafka_rest_stop_args=()
compdef "_kafka-command kafka-rest-stop" kafka-rest-stop
declare -a _kafka_server_stop_args
_kafka_server_stop_args=()
compdef "_kafka-command kafka-server-stop" kafka-server-stop
declare -a _kafka_leader_election_args
_kafka_leader_election_args=()
_kafka_leader_election_args+=('--admin.config: Configuration properties files to pass to the admin client')
_kafka_leader_election_args+=('--all-topic-partitions: Perform election on all of the eligible topic partitions based on the type of election (see the -- election-type flag). Not allowed if --topic or --path-to-json-file is specified.')
_kafka_leader_election_args+=('--bootstrap-server: A hostname and port for the broker to connect to, in the form host:port. Multiple comma separated URLs can be given. REQUIRED.')
_kafka_leader_election_args+=('--election-type: Type of election to attempt. Possible values are "preferred" for preferred leader election or "unclean" for unclean leader election. If preferred election is selection, the election is only performed if the current leader is not the preferred leader for the topic partition. If unclean election is selected, the election is only performed if there are no leader for the topic partition. REQUIRED.')
_kafka_leader_election_args+=('--help: Print usage information.')
_kafka_leader_election_args+=('--partition: Partition id for which to perform an election. REQUIRED if --topic is specified.')
_kafka_leader_election_args+=('--path-to-json-file: The JSON file with the list of partition for which leader elections should be performed. This is an example format. {"partitions": [{"topic": "foo", "partition": 1}, {"topic": "foobar", "partition": 2}] } Not allowed if --all-topic-partitions or --topic flags are specified.')
_kafka_leader_election_args+=('--topic: Name of topic for which to perform an election. Not allowed if --path-to- json-file or --all-topic-partitions is specified.')
_kafka_leader_election_args+=('--version: Display Kafka version.')
compdef "_kafka-command kafka-leader-election" kafka-leader-election
declare -a _support_metrics_bundle_args
_support_metrics_bundle_args=()
_support_metrics_bundle_args+=('--bootstrap-server: --bootstrap-server Connection string for a Kafka broker that is a member of the cluster from which metrics support will be retrieved. Example: ''localhost:9092''')
_support_metrics_bundle_args+=('--topic: --topic The Kafka topic from which the support metrics will be retrieved. Default: ''__confluent.support.metrics''')
_support_metrics_bundle_args+=('--file: --file Output filename of the support metrics bundle. Default: ''support-metrics-__confluent.support.metrics.20200315-012057.zip'' Note that, when using the default value, the timestamp is dynamically generated at each run of this tool.')
_support_metrics_bundle_args+=('--runtime: --runtime The time in seconds this tool will run for. For a large cluster you may need to increase this setting because the tool might need more time to collect all the metrics. Default: 10')
_support_metrics_bundle_args+=('--help: --help Print this help message. Important notes for running this tool: * Kafka must be up and running. * Kafka must be accessible from the machine on which this tool is executed. Copyright 2015 Confluent Inc. <http://confluent.io/>')
compdef "_kafka-command support-metrics-bundle" support-metrics-bundle
...
declare -a _ksql_test_runner_args
_ksql_test_runner_args=()
_ksql_test_runner_args+=('--input-file: --input-file <inputFile>, -i <inputFile> A JSON file containing the input records. This option may occur a maximum of 1 times')
_ksql_test_runner_args+=('--output-file: --output-file <outputFile>, -o <outputFile> A JSON file containing the expected output records. This option may occur a maximum of 1 times')
_ksql_test_runner_args+=('--sql-file: --sql-file <statementsFile>, -s <statementsFile> A SQL file containing KSQL statements to be tested. This option may occur a maximum of 1 times')
compdef "_kafka-command ksql-test-runner" ksql-test-runner
...
declare -a _connect_distributed_args
_connect_distributed_args=()
compdef "_kafka-command connect-distributed" connect-distributed
declare -a _schema_registry_stop_args
_schema_registry_stop_args=()
compdef "_kafka-command schema-registry-stop" schema-registry-stop
...
_kafka_producer_perf_test_args+=('--throughput: --throughput THROUGHPUT [--producer-props PROP-NAME=PROP-VALUE [PROP-NAME=PROP-VALUE ...]] [--producer.config CONFIG-FILE] [--print-metrics] [--transactional-id TRANSACTIONAL-ID] [--transaction-duration-ms TRANSACTION-DURATION] (--record-size RECORD-SIZE |')
_kafka_producer_perf_test_args+=('--payload-file: --payload-file PAYLOAD-FILE) This tool is used to verify the producer performance. optional arguments: -h, --help show this help message and exit')
...
declare -a _ksql_server_stop_args
_ksql_server_stop_args=()
compdef "_kafka-command ksql-server-stop" ksql-server-stop
...
_kafka_consumer_perf_test_args+=('--group: The group id to consume on. (default: perf-consumer-4318)')
...
declare -a _kafka_rest_start_args
_kafka_rest_start_args=()
compdef "_kafka-command kafka-rest-start" kafka-rest-start
...
declare -a _schema_registry_stop_service_args
_schema_registry_stop_service_args=()
compdef "_kafka-command schema-registry-stop-service" schema-registry-stop-service
declare -a _kafka_preferred_replica_election_args
_kafka_preferred_replica_election_args=()
_kafka_preferred_replica_election_args+=('--admin.config: Admin client config properties file to pass to the admin client when -- bootstrap-server is given.')
_kafka_preferred_replica_election_args+=('--bootstrap-server: A hostname and port for the broker to connect to, in the form host:port. Multiple comma-separated URLs can be given. REQUIRED unless --zookeeper is given.')
_kafka_preferred_replica_election_args+=('--help: Print usage information.')
_kafka_preferred_replica_election_args+=('--path-to-json-file: The JSON file with the list of partitions for which preferred replica leader election should be done, in the following format - {"partitions": [{"topic": "foo", "partition": 1}, {"topic": "foobar", "partition": 2}] } Defaults to all existing partitions')
_kafka_preferred_replica_election_args+=('--version: Display Kafka version.')
_kafka_preferred_replica_election_args+=('--zookeeper: DEPRECATED. The connection string for the zookeeper connection in the form host:port. Multiple URLS can be given to allow fail-over. Replaced by --bootstrap-server, REQUIRED unless --bootstrap-server is given.')
compdef "_kafka-command kafka-preferred-replica-election" kafka-preferred-replica-election
declare -a _ksql_server_start_args
_ksql_server_start_args=()
_ksql_server_start_args+=('--queries-file: --queries-file <queriesFile> Path to the query file on the local machine. -- This option can be used to separate command-line options from the list of arguments (useful when arguments might be mistaken for command-line options) <config-file> A file specifying configs for the KSQL Server, KSQL, and its underlying Kafka Streams instance(s). Refer to KSQL documentation for a list of available configs. This option may occur a maximum of 1 times')
compdef "_kafka-command ksql-server-start" ksql-server-start
...
declare -a _schema_registry_start_args
_schema_registry_start_args=()
compdef "_kafka-command schema-registry-start" schema-registry-start
...
declare -a _kafka_rest_stop_service_args
_kafka_rest_stop_service_args=()
compdef "_kafka-command kafka-rest-stop-service" kafka-rest-stop-service
declare -a _kafka_server_start_args
_kafka_server_start_args=()
_kafka_server_start_args+=('--override: Optional property that should override values set in server.properties file')
_kafka_server_start_args+=('--version: Print version information and exit.')
compdef "_kafka-command kafka-server-start" kafka-server-start
...
declare -a _ksql_args
_ksql_args=()
_ksql_args+=('--config-file: --config-file <configFile> A file specifying configs for Ksql and its underlying Kafka Streams instance(s). Refer to KSQL documentation for a list of available configs. -h, --help Display help information')
_ksql_args+=('--output: --output <outputFormat> The output format to use (either ''JSON'' or ''TABULAR''; can be changed during REPL as well; defaults to TABULAR)')
_ksql_args+=('--password: --password <password>, -p <password> If your KSQL server is configured for authentication, then provide your password here. The username must be specified separately with the -u/--user flag')
_ksql_args+=('--query-row-limit: --query-row-limit <streamedQueryRowLimit> An optional maximum number of rows to read from streamed queries This options value must fall in the following range: value >= 1')
_ksql_args+=('--query-timeout: --query-timeout <streamedQueryTimeoutMs> An optional time limit (in milliseconds) for streamed queries This options value must fall in the following range: value >= 1')
_ksql_args+=('--user: --user <userName>, -u <userName> If your KSQL server is configured for authentication, then provide your user name here. The password must be specified separately with the -p/--password flag -- This option can be used to separate command-line options from the list of arguments (useful when arguments might be mistaken for command-line options) <server> The address of the Ksql server to connect to (ex: http://confluent.io:9098) This option may occur a maximum of 1 times')
compdef "_kafka-command ksql" ksql
